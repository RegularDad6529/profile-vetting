...
### 42. Minted: LOL — paid mints dumped for dust (2026-07-13, UPDATED)
Every assessment must include "Minted: LOL": NFTs that were minted (from 0x0) — including PAID mints, not just free — and later sold or transferred for less than 0.005 ETH. The funniest ones are where they PAID to mint and still got nothing back. Calculate by: (1) find all mints from 0x0 to the wallet, (2) check if they paid ETH in the same block as the mint (mint_cost), (3) find subsequent transfers out of those same (contract, tokenID) pairs, (4) match ETH incoming in the same block as the outgoing NFT, (5) if sell price < 0.005 ETH, check for OTC payments: search all ETH txs from the NFT recipient to the wallet within ±48 hours — if ETH was sent, it's an OTC deal, NOT a LOL. Also check if the wallet sent ETH to the buyer (net payment). (6) Only count as LOL if NO payment found nearby. Report: count of true LOLs (excluding OTC), total ETH spent minting, total ETH received, net loss, worst individual offenders. Example: grubnot paid 2.78 ETH to mint 33 NFTs, sold all for 0 ETH. 4 additional items looked like LOLs but were OTC deals with separate-tx payments.