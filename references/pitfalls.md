...
### 144. Ethereum_SpokePool (ACROSS protocol) bridge transfers misidentified as art sales (2026-08-14)
Related to pitfall #125 but a newly identified bridge contract. The `Ethereum_SpokePool` contract (ACROSS protocol) moves funds cross-chain and appears as large incoming ETH transfers to the artist's wallet. Case: @HELLia had 3.43 ETH from Ethereum_SpokePool initially flagged as art sale revenue, but it was a cross-chain bridge transfer — not marketplace revenue. This inflated the perceived sales figure significantly and could change the classification from UNCLEAR to LIKELY_REAL if not caught.

**Fix**: Add `Ethereum_SpokePool` and any ACROSS protocol contract addresses to the exclusion list in pitfall #125's fix. After the script runs, categorize every incoming ETH transfer by source. The ACROSS protocol SpokePool is a bridge — ETH arriving from it represents cross-chain movement of the artist's own funds, not art sales. Cross-reference with NFT transfer timestamps: if no NFT was transferred out in the same block, it's not a sale.

### 145. Gemini API quota exhaustion (429) breaks AI artwork detection — fall back to manual visual inspection (2026-08-16)
...
### 150. Vetting script counts platform method calls as contract deployments (2026-08-26, CRITICAL)
The `seeking_nomination_vetting.py` script counts `setRoyalties`, `configureDrop`, `mintSigned`, `mintBatch`, `registerExtension`, and `createToken` method calls as "deployed contracts" (lines 336-340: `if method in (...) and from_addr == wl: deployed_contracts += 1`). These are platform interaction methods on EXISTING contracts, not contract deployments. `setRoyalties` configures royalty settings on a platform contract; `configureDrop` sets up a drop on an existing marketplace; `mintSigned` mints tokens on someone else's contract. None of these create a new contract.

**Case**: @Bubblezzz was reported as having "deployed contracts" but had ZERO actual contract deployments. The script counted `setRoyalties` and `mintSigned` calls on Chonkly/NetworkedAuctions platform contracts as deployments. This inflated the artist output assessment and could misclassify a non-deploying platform user as an artist with deployed contracts.

**Fix**: After the script runs, verify the "deployed contracts" count by checking whether the transactions actually deploy new contracts (tx `to` is null or the tx creates a contract — check receipt for `contractAddress` field). Only count transactions where a new contract was actually created. Method calls like `setRoyalties`, `configureDrop`, `mintSigned`, `mintBatch`, and `registerExtension` on existing contracts are artist ACTIVITY (configuring drops, minting), not contract DEPLOYMENT. Related to pitfall #67 (shared contracts) and #139 (factory contracts) but distinct — here the artist deployed NOTHING, the script just mislabeled method calls as deployments.

### 151. Rep categories API returns 'str' object has no attribute 'get' for some handles (2026-08-27)
The rep categories endpoint `GET /profiles/{handle}/rep/categories` can return a response where iterating over the `data` array yields string elements instead of dict objects, causing `'str' object has no attribute 'get'` when the code tries to call `.get('category')` or `.get('total_rep')` on each element. This was observed for @Uttermelon and @Ashish on Aug 27. The endpoint sometimes returns a different structure than expected — the `data` key may contain a list of category name strings rather than category objects with `category`, `total_rep`, `contributor_count`, and `top_contributors` fields.

**Fix**: Wrap the rep categories parsing in a try/except that checks `isinstance(item, dict)` before calling `.get()` on each element. If the element is a string, skip it or treat it as a category name with unknown rep. Alternatively, use the `GET /profiles/{id}/rep/categories` endpoint with the profile UUID (not handle) and paginate with `?page=N` until `next` is false, as documented in pitfall #93. The handle-based endpoint may behave differently from the ID-based one. When rep categories cannot be parsed, fall back to the identity endpoint's `rep` field for the total rep number.

### 152. Foundation marketplace payouts include collector resales, not just own-art sales (2026-08-27)
When analyzing internal ETH transfers from the Foundation v1 proxy contract (`0xcda72070e455bb31c7690a170224ce43623d0b6f`), the payouts represent ALL NFT sales routed through Foundation — including resales of collected NFTs by other artists, not just sales of the wallet owner's own art. Case: @Ashish received 1.955 ETH from Foundation (13 internal txs), but all 15 NFTs sent to Foundation were from collections he collected (WHY, DIE/Death is easy) — not from his own deployed contracts (DBP). The Foundation payouts were collector resale revenue, not artist revenue. Counting them as artist revenue would inflate the assessment.

**Fix**: When Foundation (or any marketplace) payouts are found, cross-reference the NFT transfers sent to that marketplace. Check whether the NFT contract addresses match the artist's own deployed contracts. If the NFTs sent to Foundation are from OTHER artists' contracts (collected pieces), the payouts are collector resales — exclude them from artist revenue calculations. Only count marketplace payouts as artist revenue when the NFTs sold were minted from the artist's own contracts or from drops they created (Manifold createDrop, TLStacks createDrop, SeaDrop configuration). This also applies to Seaport sales — verify the NFTs being sold are the artist's own creations, not collected pieces.

### 153. Manifold ERC1155LazyPayableClaim has multiple contract instances — payouts come from several addresses (2026-08-27)
The Manifold platform deploys MULTIPLE instances of `ERC1155LazyPayableClaim` contracts (e.g., `0x44e94034afce2dd3cd5eb62528f239686fc8f162`, `0x26bbea7803dcac346d...`, `0xe7d3982e21...`). All are named "ERC1155LazyPayableClaim" on Blockscout but are distinct contract addresses. An artist's Manifold payouts may be split across several of these instances. Case: @Ashish received Manifold 1155 payouts from THREE different contract addresses: 1.0 ETH (37 txs) from 0x44e94034, 0.273 ETH (50 txs) from 0x26bbea78, and 0.048 ETH (4 txs) from 0xe7d3982e — total 1.32 ETH across 91 txs from 3 instances.

**Fix**: When categorizing internal ETH transfers, do not hardcode a single Manifold contract address. Search for all contracts named "ERC1155LazyPayableClaim" (and "ERC721LazyPayableClaim") in the internal tx senders. Use the Blockscout smart-contracts API to identify contract names. Sum payouts across all Manifold instances to get the total Manifold revenue. The same applies to other platforms that deploy multiple proxy/implementation contracts (Seaport has at least two: 0x0000000000000068f1... and 0x00000000000000adc0...).

### 154. Blockscout v2 address counters can show 0 transactions while v1 API returns actual txs (2026-08-27)
The Blockscout v2 API `GET /api/v2/addresses/{address}/counters` can return `{"transactions_count": "0", "gas_usage_count": "0", "token_transfers_count": "0"}` for a wallet that DOES have transactions, as confirmed by the v1 API `GET /api/v1?module=account&action=txlist&address={address}` which returns actual transactions. Case: wallet `0x10b888fbb45773294d3a46b10b723c617884bad9` showed 0 on v2 counters but v1 returned 50 transactions and 36 NFT transfers (active Feb-Apr 2023). The v2 counters endpoint appears to not index older transactions or may have a lag.

**Fix**: Never rely solely on the v2 counters endpoint to determine if a wallet has on-chain activity. If v2 counters show 0, cross-check with the v1 txlist API (`module=account&action=txlist`) and the v1 tokennfttx API (`module=account&action=tokennfttx`). Only conclude "zero on-chain activity" if BOTH v1 and v2 return empty results. This is especially important for older wallets (pre-2024) where v2 indexing may be incomplete.

### 155. Profile-logs PROFILE_CREATED entry shows a different wallet than the profile's primary wallet after deconsolidation (2026-08-27)
When checking profile-logs for deconsolidated profiles, the `PROFILE_CREATED` log entry's `profile_handle` field may contain an auto-assigned `id-0x{address}` handle for a DIFFERENT wallet than the profile's current primary wallet. Case: @Uttermelon's profile-logs showed a `PROFILE_CREATED` entry with `profile_handle: 'id-0x10b888fbb45773294d3a46b10b723c617884bad9'`, but Uttermelon's primary wallet is `0xaf6e8b19e629ab3cdb8b597cc0a34ffc0e803e7d`. The `0x10b888fb` wallet was a previous/associated wallet with collector activity (36 NFT transfers in early 2023), not the current primary. The PROFILE_CREATED log actually reflects a deconsolidation event that split the profile, not the original account creation.

**Fix**: When investigating profile history via profile-logs, do not assume the wallet in the PROFILE_CREATED entry is the artist's primary or only wallet. It may be an old wallet that was deconsolidated. Always cross-reference with the `GET /identities/{handle}` endpoint's `wallets` array to see all associated wallets. If the profile-logs show PROFILE_CREATED today but the identity has 200+ active days, this confirms deconsolidation (pitfall #80). The old wallet may have on-chain activity that the current primary wallet does not — check ALL wallets in the consolidation group for on-chain history.

### 156. SeaDrop payouts (132 internal txs) can be from artist's own drop without direct SeaDrop contract interaction (2026-08-27)
An artist can receive significant SeaDrop internal transfer payouts (ETH from mints) with only 1 direct transaction to the SeaDrop contract address. Case: @Ashish had only 1 tx to the SeaDrop contract (`0x00005ea00ac477b1030ce78506496e8c2de24bf5`, method `0x161ac21f`, 0.0004 ETH) but received 132 internal payouts totaling 1.549 ETH from SeaDrop. The single tx was likely a drop configuration call, and the 132 payouts are from buyers minting the artist's drop. The payout amounts were small (0.003-0.010 ETH each), consistent with public mint pricing.

**Fix**: Do not dismiss SeaDrop (or similar drop platform) payouts just because the artist has few direct interactions with the platform contract. A single `configureDrop` or `initializeDrop` call is all that's needed — after that, every mint by a buyer generates an internal ETH transfer to the artist. High internal tx count from SeaDrop with low direct interaction = the artist created a popular drop. Verify by checking: (a) the single tx method signature matches drop configuration, (b) payout amounts are consistent with mint prices, (c) payout timestamps span a period consistent with a live drop. Count these as artist revenue.

### 157. Collector count inflated by marketplace escrow contracts (2026-07-12, CRITICAL)
NFTs sent to marketplace escrow contracts (TL Auction House, Seaport, Foundation, OpenSea listing proxies) for listing are counted as "sent to collectors" by the vetting script. The `unique_collectors` and `collector_wallets` counts are inflated because marketplace contracts and the zero address (burn/mint) are not excluded. Case: @Superno showed "40 collectors, 214 sent" but reality was ~6-7 real EOA recipients — 99 NFTs went to TL Auction House (listings), 58 to burn address (minting mechanics), 26 to OpenSea listing proxy, 16 to Foundation (listing).

**Fix**: The `get_nft_history_multichain()` function must exclude known marketplace contract addresses from `collector_wallets` and `unique_collectors` counts. Also exclude the zero address (0x000...000). After the script runs, if the collector count seems disproportionately high relative to sales, check whether marketplace escrow contracts are in the recipient list. Related to pitfall #147 (NFT self-transfers to own vault inflate sends) but distinct — here the inflation is from marketplace listing deposits, not self-vault transfers.

### 158. Blockscout API returns 403 without User-Agent header (2026-07-11)
Blockscout API requests without a `User-Agent` header can return HTTP 403 Forbidden. Add `User-Agent: Mozilla/5.0` header to all Blockscout API requests (v1 and v2 endpoints). This affects `/api?module=...` (v1) and `/api/v2/...` (v2) calls equally. Without the header, `get_tx_count()` calls time out and the vetting script blows past the cron timeout.

**Fix**: Always set `headers={'User-Agent': 'Mozilla/5.0'}` on Blockscout requests. Already applied in the vetting script (STATUS.md item 6) but documented here as a pitfall for any new scripts that interact with Blockscout.

### 159. 6529 identities wallets array uses key `wallet`, not `address` (2026-07-11)
The `GET /identities/{handle}` response includes a `wallets` array where each element has the wallet address under the key `wallet` (NOT `address`). Using `w.get("address")` silently returns `None` and produces an empty wallet list, causing the script to skip all on-chain analysis. This is a silent failure — no error is raised, just missing data.

**Fix**: Always use `w.get("wallet")` or `w["wallet"]` when iterating the `wallets` array from the identities endpoint. Verify the wallet list is non-empty after extraction. If the script reports 0 wallets for a profile that clearly has wallets, check the key name first.

### 160. Vetting script counts mint-from-0x0 as contract deployments (2026-09-01, CRITICAL)
The `seeking_nomination_vetting.py` script reports "deployed contracts" by counting NFT transfers from 0x0 (the zero address) to the wallet — i.e., minting tokens. But minting from a contract does NOT mean the wallet deployed that contract. The script does not check `creator_address_hash` on the contract. Case: @mndigiart6 was reported as having "7 deployed contracts" but had ZERO actual deployments — the main collection "The 6ixth Plane" was created by a different address (0x612E2D...). The 7 "deployments" were just mints from contracts deployed by others.

**Fix**: After the script runs, ALWAYS verify "deployed contracts" by checking `GET /v2/addresses/{contract}` and confirming `creator_address_hash` matches one of the profile's wallets. Only contracts where the creator IS the profile wallet count as deployed. All others are contracts the artist minted ON, not contracts they created. Related to pitfall #67 (shared/factory contracts) and pitfall #150 (method calls mislabeled as deployments), but this is a distinct script failure mode — the script conflates "minted from 0x0 on this contract" with "deployed this contract."

### 161. Shared cashout address between artist and buyer indicates wash trading (2026-09-02, CRITICAL)
An artist and their buyer may both send ETH to the same intermediary address, which then cashes out to a centralized exchange (Binance, Coinbase). This circular pattern means the "sales" are wash trades — the money returns to the same person. Case: @debycloud received 0.076 ETH from buyer 0x785d575c for NFT purchases, but BOTH debycloud AND the buyer separately sent ETH to 0x612d5f6e (only 5 total txs, only 2 senders). 0x612d5f6e then sent 0.1467 ETH to Binance (0x28c6c062). The total cashout (0.1487 ETH) nearly equals the combined inflows from both parties. This is NOT a platform fee address (those have many senders); it's a shared personal cashout.

**Fix**: For each candidate, trace ETH outflows from BOTH the artist's wallet AND their buyers' wallets. If they share a cashout address (an EOA with few txs that receives from both and sends to an exchange), flag the sales as suspicious/wash trades. Distinguish from platform fee addresses by checking sender count: platform addresses have 7+ unique senders, shared personal cashouts have 2-3. Do NOT give rep to artists with this pattern. Related to pitfall #3 (self-transfers) but different — here the artist and buyer are supposedly different people sharing a cashout, not obvious self-transfers.

### 162. Gemini model names become deprecated (404) — call list_models() before artwork analysis (2026-09-02)
Gemini API models are deprecated over time and return 404 "model not found" errors. As of Sep 2026, `gemini-1.5-flash` and `gemini-2.0-flash` both return 404. The agent wasted several API calls trying deprecated models before discovering available ones. Pitfall #145 covers quota exhaustion (429) but not model deprecation (404).

**Fix**: Before artwork analysis, call `genai.list_models()` and filter for flash models with `generateContent` support. Use the latest available model (e.g., `gemini-2.5-flash` as of Sep 2026). Do NOT hardcode model names in scripts — always discover them dynamically. The `.env` file's `AUXILIARY_VISION_MODEL` setting may also be outdated; verify it against the list_models() output. If the configured model returns 404, fall back to the newest flash model from the list.

### 163. 0x6f66b95a is a platform-wide Transient.xyz collector, not a single artist's vault — correction to pitfall #147 (2026-09-02)
...
### 166. Candidates assessed without rep are not tracked by the vetting script's "already vetted" filter — causes re-assessment and duplicate rep (2026-09-16, CRITICAL)
The `seeking_nomination_vetting.py` candidate filter builds `vetted_handles` from only two state file keys: `rep_given` and `replies_posted`. When a candidate is assessed but no rep is given (already above the 50K MemesNominee threshold), the state file stores them under a separate top-level key (e.g., `{"OMbuilder": {"vetted": true, "rep_given": false, ...}}`). On the next vetting run, the script does not find this handle in `rep_given` or `replies_posted`, so it classifies them as a "NEW candidate" again — leading to a full re-assessment and potentially giving unnecessary rep. Case: @OMbuilder was assessed on Sep 15 (ESTABLISHED, MemesNominee rep at 53K, no rep given, state file updated correctly). On Sep 16, OMbuilder appeared as a "NEW candidate" again and was given 10,000 rep — despite already being above the threshold. The Sep 16 session also re-assessed @Soliiiart and @Restless_M, but those were genuinely new.

**Fix**: The vetting script's `vetted_handles` filter must also scan top-level keys in the state file that have a `"vetted": true` field, not just `rep_given` and `replies_posted`. Alternatively, always add assessed-but-no-rep candidates to `rep_given` with `"amount": 0` so they are caught by the existing filter. Until the script is fixed, the agent must manually check the state file for any top-level handle entries before processing "NEW candidates" from the script output.