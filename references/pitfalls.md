...
Every assessment must include "Minted: LOL": NFTs that were minted (from 0x0) — including PAID mints, not just free — and later sold or transferred for less than 0.005 ETH. The funniest ones are where they PAID to mint and still got nothing back. Calculate by: (1) find all mints from 0x0 to the wallet, (2) check if they paid ETH in the same block as the mint (mint_cost), (3) find subsequent transfers out of those same (contract, tokenID) pairs, (4) match ETH incoming in the same block as the outgoing NFT, (5) if sell price < 0.005 ETH, it's a LOL. Report: count of paid-mint LOLs separately from free-mint LOLs, total ETH spent minting, total ETH received from selling, net loss, and the worst individual offenders (highest mint cost → 0 ETH sale). Example: grubnot paid 3.33 ETH to mint 37 NFTs, sold them all for 0 ETH. Worst: Invisible Friends #3826 (0.25 ETH mint → 0 ETH sale).